...
export default function FlashcardsPage() {
  ...
  const exampleFlashcards = [
    ...
    { id: '11', question: "What is the boiling point of water?", answer: "100°C" },
  ...
  // Split flashcards into free and premium
  const freeFlashcards = flashcards.slice(0, 10);
  const premiumFlashcards = flashcards.slice(10);
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.flashcardsContainer}>
        {/* Display free flashcards */}
        {freeFlashcards.map((card) => (
        ...
        {/* Display premium flashcards if purchased */}
        {purchased && premiumFlashcards.map((card) => (
          <div key={card.id} className={styles.flashcard}>
            <div className={styles.flashcardContent}>
              <p className={styles.question} onClick={() => toggleAnswer(card.id)}>
                {card.question}
      ...
      {!purchased && (
        ...
        >
          Purchase Access for $5 to Unlock All Flashcards
        </button>
      )}
      <button 
